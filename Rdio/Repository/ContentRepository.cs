...
namespace Rdio.Repository
{
    public class ContentRepository
    {
        ...
        public async Task<IEnumerable<Models.Content.NewsContent>> GetBlockContent(List<string> RssId,int Count)
        {
            ...
            try
            {
                var _model = await NoSql.Instance.RunCommandAsync<BsonDocument>("{aggregate:'content',pipeline:[{$match:{'rssid':{$in:"+RssId.toJSON()+ "},'titr':{ $ne: '' }}},{$sort : { 'createdateticks' : -1  }},{$limit:" + Count + "}]}");
                if (_model.GetValue("result").AsBsonArray.Any())
